...
def funcCall(cur, conn):
    try:
        ...
        cur.execute(''' call upag.sp_checkdtlatestestimatedata_v4() ''')
        conn.commit()
        return {'success': True}
...
def pipeline_handler(connection,connection_type):
    log_file = f"dynamicdatatriangulationv4{(connection_type).lower()}.log"
    logging.basicConfig(filename = log_file, filemode='w', level=logging.INFO,
        format='%(asctime)s - %(name)-12s - %(levelname)-4s - %(filename)s - %(funcName)s -%(lineno)d - %(message)s')
    start_time_stamp = datetime.now(pytz.timezone('Asia/Kolkata'))
    log.info(f" Dyanmic data triangulation reports v1 ETL job triggered at {start_time_stamp}")
    ...
    if resp['success']:
        log.info(f"{connection_type} Dynamic data triangulation V4 job ended at {end_timestamp}")
        subject = f"INFO : {connection_type} Dynamic data triangulation job V4 status"
        body = f"{connection_type} Dynamic data triangulation job v4 succeeded. Please check the attached logs for details."
    else:
        log.critical(f"{connection_type} Dynamic data triangulation V4 job ended at {end_timestamp}")
        subject = f"CRITICAL : {connection_type} Dynamic data triangulation job V4 status"
        body = f"{connection_type} Dynamic data triangulation job V4 failed. Error : {resp['message']} Please check the attached logs for details."
