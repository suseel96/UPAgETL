...
import pytz
import math
from etl_utils.utils import emailClient, fileUploadUtils, dbClients
...
class dcsETL:
  ...
  def requestSeekAPI(self, year, state, season, page_size, page_num):
    ...
    response = seek_api_resp['response']
    headers = seek_api_resp['headers']
    payload = seek_api_resp['payload'] 
...
def pipeline_handler(connection,connection_type):
  ...
  log.info(f"DCS ETL job started at {start_time_stamp}")
  ...
  years = ['2023-2024']
  states_list = [21, 23, 24, 9]
  season_list = ['Kharif', 'Rabi', 'Zaid']
  for year in years:
    for state in states_list:
      for season in season_list:
        log.info(f'Requesting page_count for state_code {state} for {year}, {season}')
        resp = dcsETL(engine,conn,cur).requestSeekAPI(year = year, state = state, season = season, page_size=1, page_num=1)
  dcsETL(engine,conn,cur).requestDataInPages()
  dcsETL(engine,conn,cur).processDataInPages()
  dcsETL(engine,conn,cur).loadDataToReporting()
