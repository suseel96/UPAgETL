...
class sopaPricesETL():
    ...
    def generateOilDates(self):
        try:
            ...
            if solvent_oil_date == refined_oil_date:
                dates = pd.date_range(start = solvent_oil_date + timedelta(days = 1 ) , end = datetime.now().date())
                dates = [datetime.strftime(date,'%Y-%m-%d') for date in dates]
                return {'success':True, 'dates':dates}
            else:
                log.info('Please check dates are not equal for two oils ')
                return {'success':False,'message':'Please check dates are not equal'}
    ...
    def scrapeOilsData(self,dates):
        try:
            log.info("Started Scraping Oils Data")
            ...
            for solvent, crop in zip(('solvent','refined'),('soybean','refined')):
                for date in dates:
                    browser.get(f'https://www.sopa.org/daily-rates/{crop}-oil/?solvoil={solvent}-oil&search_type=search_by_date&single_date={date}&starting_year_value=&ending_year_value=&station=Indore&submit=Search')
            ...
            log.critical('Error in '+self.scrapeOilsData.__name__+f': {e}')
            return {'success' : False, 'message' : 'Error in '+self.scrapeOilsData.__name__+f': {e}'}
    ...
    def loadDataToStaging(self, oils_df , meal_df):
        try:
    ...
    def startSopaPricesETL(self):
        try:
            ...
            if oil_dates_resp['success']:
                oil_dates = oil_dates_resp['dates']
            scrape_oil_data_resp = self.scrapeOilsData(oil_dates)
            if scrape_oil_data_resp['success']:
                oil_columns_resp = commonUtills().formatColumnNames(scrape_oil_data_resp['data'])
                if oil_columns_resp['success']:
                    oils_df = oil_columns_resp['data']            
            meal_dates_resp = self.generateMealDates()
            if meal_dates_resp['success']:
                meal_dates = meal_dates_resp['dates']
            scrape_meal_data_resp = self.scrapeMealData(meal_dates)
            if scrape_meal_data_resp['success']:
                meal_column_resp = commonUtills().formatColumnNames(scrape_meal_data_resp['data'])
                if meal_column_resp['success']:
                    meal_df = meal_column_resp['data']
            load_data_to_staging_resp = self.loadDataToStaging(oils_df , meal_df)
...
def pipeline_handler(connection,connection_type):
    ...
    log.info(f"SOPA ETL job triggered at {datetime.now(pytz.timezone('Asia/Kolkata'))}")
    ...
    if resp['success']:
        log.info(f"{connection_type} SOPA ETL job ended at {datetime.now(pytz.timezone('Asia/Kolkata'))}")
        ...
        log.critical(f"{connection_type} SOPA ETL job ended at {datetime.now(pytz.timezone('Asia/Kolkata'))}")
    ...
    log.info(f"Execution time of sopa{(connection_type).lower()}etl is {str(round(total_time/60,3))} Minutes")
