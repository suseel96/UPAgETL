...
#                 DOCA WHOLESALE PRICES DAILY ETL JOB
...
#ETL CODE
class docaWholesalePricesETL:
    def __init__(self,date,engine,conn,cur):
        # self.date = (datetime.now().date()-timedelta(days=2)).strftime('%d-%m-%Y')
        self.date = date
        self.engine = engine
        self.conn = conn
        self.cur = cur
        self.DB_STAGING_SCHEMA = 'staging'
        self.DB_STAGING_TABLE_NAME = 'stgDocaWholesalePrices'
        self.DB_REPORTING_SCHEMA = 'reporting'
        self.DB_REPORTING_TABLE_NAME = 'RptDocaWholesalePrices'
    ...
    def loadDataToReporting(self):
        try:
            ...
            self.cur.execute(f'''
            select "QueryText" from reporting."ETLFlowMaster" where 
            "DataSource" = 'DocaWholesale' and "UpdateMode" = 'Delta'and "SeqNo" =2;
            ''')
    ...
    def loadDataToMIP(self):
        try:
            ...
            self.cur.execute(f'''
            select "QueryText" from reporting."ETLFlowMaster" where 
            "DataSource" = 'DocaWholesale' and "UpdateMode" = 'Delta'and "SeqNo" =3;
            ''')
            query_text = self.cur.fetchall()[0][0]
            query_date = pd.to_datetime(self.date, format='%m/%d/%Y').strftime('%Y-%m-%d')
            self.cur.execute(query_text, {'date_param':query_date})
    ...
    def startDocaWholesaleETL(self):
        try:
            ...
            log.critical('Error in '+self.startDocaWholesaleETL.__name__+f': {e}')
            return {'success': False, 'message' : 'Error in '+self.startDocaWholesaleETL.__name__+f': {e}'}

def maxDate(cur):
    cur.execute('''
                   select max("Date") from staging."stgDocaWholesalePrices"
    ''')
...
def delFiveDays(cur,conn):
        try:
            cur.execute(f'''
            select "QueryText" from reporting."ETLFlowMaster" where "DataSource" = 'DocaWholesale' 
...
def pipeline_handler(connection,connection_type):
    log_file = f"docawholesale{(connection_type).lower()}etl.log"
    logging.basicConfig(filename = log_file, filemode='w', level=logging.INFO,
        format='%(asctime)s - %(name)-12s - %(levelname)-4s - %(filename)s - %(funcName)s -%(lineno)d - %(message)s')
    start_time_stamp = datetime.now(pytz.timezone('Asia/Kolkata'))
    log.info(f"Doca Wholesale Prices ETL job triggered at {start_time_stamp}")
    ...
    if dates:
        for date in dates:
            resp = docaWholesalePricesETL(date,engine,conn,cur).startDocaWholesaleETL()
            end_timestamp = datetime.now(pytz.timezone('Asia/Kolkata'))
            if resp['success']:
                log.info(f"{connection_type} Doca Wholesale Prices ETL job ended at {end_timestamp}")
                subject = f"INFO : {connection_type} Doca Wholesale Prices ETL job status"
                body = f"{connection_type} Doca Wholesale Prices ETL job succeeded. Please check the attached logs for details."
            else:
                log.critical(f"{connection_type} Doca Wholesale Prices ETL job ended at {end_timestamp}")
                subject = f"CRITICAL : {connection_type} Doca Wholesale Prices ETL job status"
                body = f" {connection_type} Doca Wholesale Prices ETL job failed. Error : {resp['message']} Please check the attached logs for details."
        total_time = ((end_timestamp - start_time_stamp).total_seconds())/60        
        log.info(f"Execution time of docawholesale{(connection_type).lower()}etl is {round(total_time,3)} Minutes")
